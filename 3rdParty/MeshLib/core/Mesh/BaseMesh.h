...
public:
    ...
    */
    std::map<int, CVertex*> & mapvertes() { return m_map_vert; };

    std::map<int, CFace*>& mapfaces() { return m_map_face; };
...
inline CEdge * CBaseMesh<CVertex, CEdge, CFace, CHalfEdge>::vertexEdge(tVertex  v0, tVertex  v1)
{
    ...
    for (typename std::list<CEdge*>::iterator eiter = ledges.begin(); eiter != ledges.end(); eiter++)
    {
        ...
        //cout << pH->source()->id() << " " << pH->target()->id() << endl;
...
void CBaseMesh<CVertex, CEdge, CFace, CHalfEdge>::read_m(const char * input)
{
    ...
    while (is.getline(buffer, MAX_LINE))
    {
        ...
        if (token == "Face")
        {
            ...
            */
            ...
            if (sp >= 0 && ep >= 0)
            {
                ...
                //std::cout << "f->string(): " << f->string() << std::endl;
            }
            /*
        ...
        if (token == "HalfEdge")
        {
            stokenizer.nextToken();
            token = stokenizer.getToken();
            int vSourceId = strutil::parseString<int>(token);

            stokenizer.nextToken();
            token = stokenizer.getToken();
            int vTargetId = strutil::parseString<int>(token);



            /*H* he = NULL;
            V* v = this->idVertex(vSourceId);
            for()
            for (auto vh : It::VCcwOutHEIterator(this, v))
            {
                if (vh->target()->id() == vTargetId())
                {
                    he == vh;
                }
            }*/
            //CFace* f = idFace(fid);
            //tHalfEdge he = corner(v, f);
            CVertex* heS = this->idVertex(vSourceId);
            CVertex* heT = this->idVertex(vTargetId);
            CHalfEdge* he = vertexHalfedge(heS, heT);
            ...
            {
                he->string() += token.substr(sp + 1, ep - sp - 1);
                //std::cout << "he->string(): " << he->string() << std::endl;
            }
            continue;
        }
    }
...
CFace * CBaseMesh<CVertex, CEdge, CFace, CHalfEdge>::createFace(std::vector<tVertex> &  v, int id)
{
    ...
    for (size_t i = 0; i < hes.size(); i++)
    {
        ...
        else
        {
            ...
            {
                tVertex v1 = this->edgeVertex1(e);
                tVertex v2 = this->edgeVertex2(e);
