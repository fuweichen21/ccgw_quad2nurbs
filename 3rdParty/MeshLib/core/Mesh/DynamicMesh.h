...
//#include <PreMeshlib/core/Mesh/boundary.h>
#include"../3rdParty/MeshLib/core/Mesh/boundary.h"
//#include <PreMeshlib/core/Mesh/iterators.h>
#include"../3rdParty/MeshLib/core/Mesh/iterators.h"
...
CVertex * CDynamicMesh<CVertex,CEdge,CFace,CHalfEdge>::splitFace( CFace * pFace )
{
	...
	CVertex * pV = this->createVertex( ++m_vertex_id );
...
void CDynamicMesh<CVertex,CEdge,CFace,CHalfEdge>::swapEdge( CEdge * edge )
{
  ...
  CHalfEdge * he_left   = this->edgeHalfedge( edge, 0 );
  CHalfEdge * he_right  = this->edgeHalfedge( edge, 1 );
  ...
  CVertex * v1 = this->edgeVertex1( edge );
  CVertex * v2 = this->edgeVertex2( edge );
  ...
  ph[0] = he_left;
  ph[1] = this->faceNextCcwHalfEdge( ph[0] );
  ph[2] = this->faceNextCcwHalfEdge( ph[1] );


  ph[3] = he_right;
  ph[4] = this->faceNextCcwHalfEdge( ph[3] );
  ph[5] = this->faceNextCcwHalfEdge( ph[4] );

  CVertex * pv[4];

  pv[0] = this->halfedgeTarget( ph[0]);
  pv[1] = this->halfedgeTarget( ph[1]);
  pv[2] = this->halfedgeTarget( ph[2]);
  pv[3] = this->halfedgeTarget( ph[4]);
...
*/
  ...
  {
	  CHalfEdge * h = ph[i];
	  CVertex   * v = this->halfedgeTarget(h);
	  v->halfedge() = h;
  }
...
CVertex * CDynamicMesh<CVertex,CEdge,CFace,CHalfEdge>::splitEdge( CEdge * pEdge )
{
	...
	h[0] = this->edgeHalfedge( pEdge, 0 );
	h[1] = this->faceNextCcwHalfEdge( h[0] );
	h[2] = this->faceNextCcwHalfEdge( h[1] );

	h[3] = this->edgeHalfedge( pEdge, 1 );
	h[4] = this->faceNextCcwHalfEdge( h[3] );
	h[5] = this->faceNextCcwHalfEdge( h[4] );


	CFace * f[4];
	f[0] = this->halfedgeFace( h[0] );
	f[1] = this->halfedgeFace( h[3] );

	for( int i = 0; i < 6; i ++ )
	{
		eg[i] = this->halfedgeEdge( h[i] );
		v[i] = this->halfedgeVertex(h[i]);
		s[i] = this->halfedgeSym( h[i] );
	}
	...
	for( int k = 0; k < 4; k ++ )
	{
		CHalfEdge * pH = this->faceHalfedge( f[k] );
		for( int i = 0; i < 3; i ++ )
		{
			assert( pH->vertex() == pH->he_sym()->he_prev()->vertex() );
			pH = this->faceNextCcwHalfEdge( pH );
		}
	}
	return pV;	
